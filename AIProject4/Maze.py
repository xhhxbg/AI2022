...
class Maze(object):
    ...
    def __init__(self, maze_size=5):
        ...
        self.valid_symbols = ['↑', '→', '↓', '←']  # Up, Right, Down, Left
    ...
    def show_dir(self, Q_table):
        self.draw_maze()
        for loc in Q_table:
            action = self.valid_symbols[np.argmin(Q_table[loc]).item()]
            x, y = loc
            plt.text(x+0.5, y+0.5, action)
        plt.show()
    ...
    def move_robot(self, direction):
        ...
        if self.is_hit_wall(self.robot['loc'], direction):
            ...
            if self.robot['loc'] == self.destination:
                ...
                reward = self.reward['start_point']
            else:
