...
class Runner(object):
    ...
    def train_and_test(self, training_per_epoch, testing_per_epoch, epoch_limit=10000):
        """initialization"""
        success = False
        self.robot.reset()

        """explore the map"""
        self.robot.mod = 'explore'

        while True:
            self.robot.train_update()
            if self.maze.sense_robot() == self.maze.destination:
                print("Explore reach destination")
                print(f"memory size: {len(self.robot.memory)}")
                self.robot.mod = 'train'
                break
        ...
        for e in range(epoch_limit):
            ...
            self.robot.epsilon = self.robot.epsilon0
            
            self.robot.reset()
            p_bar = tqdm(
                total=training_per_epoch,
                desc="Trainning...",
            )
            for i in range(training_per_epoch):
            ...
            for i in range(testing_per_epoch):
                self.robot.test_update()
                if self.maze.sense_robot() == self.maze.destination:
                    success = True
                    break

            self.robot.show_Q(print_flag=False)

            # self.robot.show_Q(True)
            # self.robot.maze.show_dir(self.robot.q_table)
