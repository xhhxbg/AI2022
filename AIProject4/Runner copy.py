    ...
    def run_training(self, training_epoch, training_per_epoch=150):
        ...
        while True:
            ...
            while True:
                ...
                if current_record['state'] not in states:
                    states.append(current_record['state'])
            ...
            p_bar.close()
            print("Trained until succeed.")
            self.add_statics(accumulated_reward, run_times)

            self.robot.reset()
            """ show Q table """
            Q_tabel = self.robot.calc_Q()
            self.robot.show_Q()
            self.maze.show_locations(Q_tabel)
            """ test """
            self.run_testing(e)
            if self.maze.sense_robot() == self.maze.destination:
                ...
                print("Defeat!", self.maze.sense_robot())

            e += 1
            

    def run_testing(self, e):
